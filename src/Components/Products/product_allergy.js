...
export default function ProductAllergy() {
  const [data, setData] = useState([]);
  const [nutriID, setNutriID] = useState('');
  const [scale, setScale] = useState();
  const [save, setSave] = useState(false);
  const match = useRouteMatch('/product_allergy/:id');
    const [nutri, setNutri] = useState([]);
    const [del, setDel] = useState(false);
  const productID = match.params.id;
  useEffect(
    () => {
        axios.get("http://localhost:8000/api/allergy/getAllergies", {
            headers: {
        ...
        axios.get("http://localhost:8000/api/getAllergy/"+productID, {
          headers: {
              Accept: 'application/json',
              'Content-Type': 'application/json',
           //  Authorization: 'Bearer ' + Token,
  ...
  function SaveNutri(event) {
    ...
    axios.post('http://localhost:8000/api/AllergyProd/create',
        {
  ...
  function onDelete(id) {
    axios.delete("http://localhost:8000/api/AllergyProd/delete/" + id).then((response) => {
        alert(response.data)
        setDel(!del);
        ...
        <div className="container" >
            <div className="row justify-content-center">
                <div className="col-md-8">
                    <div className="card">
                        <div className="card" style={{ textAlign: 'center',fontWeight:'bold',justifyContent:'center', alignItems:'center' ,color:' rgba(233, 72, 28, 1)',fontSize:'50px' ,padding:'10px',borderRadius:'2px'}}>Add New Allergy</div>
